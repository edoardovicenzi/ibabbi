...
import { selectSecretFriends } from "../utilities/array";
...
export default function CreateList({ festivity = "christmas" }) {
  ...
  async function handleSubmit(e) {
    ...
    const data = {
      ...
      names: selectSecretFriends(listaNomi, 5),
    };
  ...
  return (
    <div className="card-list-wrapper">
      <div className="card-list-enter-code">
        ...
        <p>
          <i>
            Inserisci il codice per scoprire il tuo{" "}
            {festivity === "christmas"
      ...
      <form className="card-list-form" onSubmit={handleSubmit}>
        ...
        <div className="card-list-add">
          <input
            ...
            placeholder={
              listaNomi.length
                ? "Inserisci il prossimo nome"
                : "Inserisci il tuo nome"
            }
            value={nuovoNome}
            onKeyDown={(e) => {
              if (e.key === "Enter") return addName();
            }}
        ...
        {!validation && (
          <ValidationError msg="Devi aggiungere almeno 3 persone!" />
        )}
        {listaNomi.length > 0 && (
          <ul className="card-list-ul">
            {listaNomi.map((el) => (
              <li className="card-list-item" key={el.id}>
                {el.name}{" "}
                <button
