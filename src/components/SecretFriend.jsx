...
export async function loader({ params }) {
  ...
  return await getNames("easter", params.id);
}
  ...
  function handleClick(nameId) {
    if (!secretFriend) {
      ...
      localStorage.setItem(
        "ibabbi",
        JSON.stringify({
          ...JSON.parse(localStorage.getItem("ibabbi")),
          [id]: friendData,
        }),
      );
    }
  ...
  return (
    <div className="secret-friend-view-wrapper">
      ...
      <div className="secret-friend-share">
        ...
        <p>{id}</p>
        {actionsState && (
          <p className="p-actions-confirmation">{actionsState}</p>
        )}
        <div className="share-url">
          <button
            className="btn bx bx-copy"
            onClick={() => handleShare(ACTIONS.copy)}
          ></button>
          {isMobile && (
            <button
              className="btn bx bx-share-alt"
              onClick={() => handleShare(ACTIONS.share)}
            ></button>
          )}
          <button
            className="btn bx bx-link"
            onClick={() => handleShare(ACTIONS.link)}
          ></button>
        </div>
      </div>
    </div>
